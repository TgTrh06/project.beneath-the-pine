...
export const createPactSchema = z.object({
    participantIds: z.array(idSchema).min(1).max(7),
    startsAt: dateTimeSchema,
    durationMinutes: durationMinutesSchema
}).strict();

export const respondPactSchema = z.object({
    response: z.enum(["accepted", "declined"])
}).strict();

export const pactParticipantSchema = z.object({
    accountId: idSchema,
    displayName: z.string().nullable(),
    response: pactResponseSchema
});
...
export const pactSchema = z.object({
    ...
    participants: z.array(pactParticipantSchema),
    createdAt: dateTimeSchema
});

export type FocusPact = z.infer<typeof pactSchema>;

export const pactListQuerySchema = z.object({
    group: z.enum(['pending', 'upcoming', 'active', 'past']).default('pending'),
...
export type PactListItem = {
    id: string;
    circleId: string;
    circleName: string;
    startsAt: string;
    durationMinutes: number;
    status: z.infer<typeof pactStatusSchema>;
    response: z.infer<typeof pactResponseSchema>;
    isCreator: boolean;
