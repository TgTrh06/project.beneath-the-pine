...
type Account = { id: string; email: string; passwordHash: string; createdAt: Date };
...
class MemoryStore {
  ...
  async createAccount(email: string, passwordHash: string) {
    ...
    const value: Account = { id: randomUUID(), email, passwordHash, createdAt: new Date() };
  ...
  async findSession(hash: string) {
    ...
    return { session, account: owner ? { id: owner.id, email: owner.email } : null };
  }
  ...
  async deleteAccount(accountId: string) { this.accounts = this.accounts.filter(value => value.id !== accountId); }
}
...
test('registration normalizes email, rotates session and returns a safe account DTO', async t => {
  const { agent, anon, register, store } = await setup(t);
  const result = await register('  Wanderer@Example.test ').expect(201);
  assert.equal(result.body.account.email, 'wanderer@example.test');
  assert.equal(result.body.account.role, undefined);
  ...
  assert.equal(current.body.account.id, result.body.account.id);
  ...
  await agent.post('/api/v1/auth/register').set('Origin', origin).set('X-XSRF-TOKEN', result.body.csrfToken)
    .send({ email: 'other@example.test', password, role: 'operator' }).expect(400);
  await agent.post('/api/v1/auth/register').set('Origin', origin).set('X-XSRF-TOKEN', result.body.csrfToken)
...
test('login errors do not disclose account existence; logout and expiry revoke access', async t => {
  ...
  await request(server).get('/api/v1/me/profile').set('Cookie', cookie).expect(401);
  await agent.post('/api/v1/auth/logout').set('Origin', origin).set('X-XSRF-TOKEN', logged.body.csrfToken).expect(204);
  await request(server).get('/api/v1/me/profile').set('Cookie', logged.headers['set-cookie'][0].split(';')[0]).expect(401);
  const anon = await agent.get('/api/v1/auth/session').expect(200);
  ...
  await agent.get('/api/v1/me/profile').expect(401);
});
test('identity separation is server enforced and the removed admin route stays absent', async t => {
  const { agent, server, register } = await setup(t);
  await request(server).get('/api/v1/me/profile').set('X-User-ID', 'admin').set('Authorization', 'Bearer admin').expect(401);
  await register('owner@example.test').expect(201);
  await agent.get('/api/v1/admin/accounts').expect(404);
  ...
  assert.notEqual(first.body.account.id, second.body.account.id);
  assert.equal(first.body.account.email, 'owner@example.test');
});
test('password hashing verifies without exposing or reusing salts', async () => {
