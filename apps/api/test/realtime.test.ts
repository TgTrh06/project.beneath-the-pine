...
test('realtime authenticates Origin/session, authorizes subscribe and returns durable snapshot', async t => {
  ...
  const identity = { sessionFromCookie: async (cookie?: string) => cookie === 'BTP_SESSION=valid' ? { account: { id: '9e52c17c-999d-4add-88b7-89c1b63a79ef', email: 'member@example.test' } } : undefined };
  ...
  const denied = io(url, { transports: ['websocket'], reconnection: false, extraHeaders: { Origin: 'https://evil.test', Cookie: 'BTP_SESSION=valid' } }); t.after(() => denied.close()); await waitFor(denied, 'disconnect');
});
