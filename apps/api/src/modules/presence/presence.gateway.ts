...
export class PresenceGateway implements OnGatewayConnection, OnGatewayDisconnect, OnModuleInit, OnModuleDestroy {
  ...
  async handleConnection(socket: PineSocket) { const origin = socket.handshake.headers.origin; const current = origin === this.config.API_WEB_ORIGIN ? await this.identity.sessionFromCookie(socket.handshake.headers.cookie) : undefined; if (!current?.account) return socket.disconnect(true); socket.data.account = current.account; socket.data.sessions = new Set(); }
