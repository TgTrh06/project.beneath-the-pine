...
export function PactView({ id, accountId, onSession }: { id: string; accountId: string; onSession: (id: string) => void; onNotice: (text: string) => void }) {
  ...
  const act = async (action:()=>Promise<unknown>) => { if (busy) return; setBusy(true); setError(""); try { await action(); } catch(e) { setError((e as Error).message); await resource.reload(); } finally { setBusy(false); } };
  ...
  return <section className="feature-card narrow-content"><p className="eyebrow">FOCUS PACT · {status[pact.status]}</p><h1>{pact.durationMinutes} phút có mặt cùng nhau.</h1><p>{formatTime(pact.startsAt,resource.data!.timezone)} · {resource.data!.timezone}</p><ErrorMessage message={error || resource.error} retry={resource.reload}/>{message && <p role="status">{message}</p>}<ul className="row-list">{pact.participants.map(person => <li key={person.accountId}>{person.displayName ?? "Wanderer"}{person.accountId === accountId ? " · Bạn" : ""}<p className="field-hint">{responses[person.response]}</p></li>)}</ul>
    ...
    <div className="button-row"><button className="secondary" disabled={busy || resource.loading} onClick={() => void resource.reload()}>Cập nhật trạng thái</button><button className="text-action" onClick={() => void act(async () => { await navigator.clipboard.writeText(location.origin+location.pathname+"#pact/"+id); setMessage("Đã sao chép link. Chỉ người được chọn mới truy cập được."); })}>Sao chép link Pact</button></div>
