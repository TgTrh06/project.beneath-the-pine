  ...
  return <div className="site-shell">
    ...
    <main className="main-layout">
      ...
      <section className="content" aria-live="polite">
        ...
        {view === "now" && <NowView task={activeTask} running={running} format={format} helpSuggestion={helpSuggestion} onCapture={() => navigate("capture")} onStart={(task) => void start(task)} onPause={() => setRunning(false)} onComplete={() => void complete()} onSmaller={(task) => void askForHelp(task)} onAcceptHelp={acceptHelp} onReset={reset} />}
...
function NowView({ task, running, format, helpSuggestion, onCapture, onStart, onPause, onComplete, onSmaller, onAcceptHelp, onReset }: { task?: Task; running: boolean; format: string; helpSuggestion: { taskId: string; title: string; minutes: number } | null; onCapture: () => void; onStart: (task: Task) => void; onPause: () => void; onComplete: () => void; onSmaller: (task: Task) => void; onAcceptHelp: () => void; onReset: () => void }) {
  return <><section className="hero"><p className="eyebrow">KHI MỌI THỨ ĐANG HƠI NHIỀU</p><h1>Bạn không cần xử lý hết.<br />Mình chỉ tìm một bước tiếp theo.</h1><p>Không cần viết đẹp hay sắp xếp trước. Bắt đầu bằng điều đang chiếm tâm trí bạn nhất.</p><button className="primary" onClick={onCapture}>Trút bớt trong đầu <span>→</span></button></section>
    ...
    <section className="two-up"><article className="feature-card timer-card"><p className="eyebrow">PHIÊN BẮT ĐẦU</p><div className="timer">{format}</div><p>{task?.title ?? "Chọn một bước phía trên để bắt đầu."}</p><div className="button-row"><button className="primary" disabled={!task} onClick={() => task && onStart(task)}>{running ? "Đang tập trung" : "Bắt đầu"}</button><button className="secondary" disabled={!task} onClick={onComplete}>Đã xong</button></div></article><article className="feature-card reset-card"><p className="eyebrow">KHI KẾ HOẠCH VỠ</p><h2>Reset hôm nay</h2><p>Chọn lại điều còn thực tế với năng lượng và thời gian hiện tại.</p><button className="text-action" onClick={onReset}>Lập lại nhẹ nhàng →</button></article></section></>;
}

function CaptureView({ dump, setDump, suggestions, onCapture, onChoose }: { dump: string; setDump: (value: string) => void; suggestions: Task[]; onCapture: () => void | Promise<void>; onChoose: (task: Task) => void | Promise<void> }) { return <><section className="page-intro"><p className="eyebrow">BRAIN DUMP</p><h1>Cứ đặt xuống đây.</h1><p>AI sẽ đề xuất vài bước nhỏ; không có gì được lưu thành task cho đến khi bạn chọn.</p></section><section className="feature-card"><label htmlFor="brain-dump">Điều đang chiếm tâm trí bạn</label><textarea id="brain-dump" value={dump} onChange={(e) => setDump(e.target.value)} placeholder="Ví dụ: phải nộp báo cáo; gọi cho mẹ; phòng hơi bừa; chưa biết bắt đầu từ đâu…" /><button className="primary full" onClick={() => void onCapture()}>Gợi ý một bước đầu tiên →</button></section>{suggestions.length > 0 && <section className="suggestions"><p className="eyebrow">BẠN CHỌN, AI KHÔNG QUYẾT ĐỊNH THAY</p>{suggestions.map((task) => <article className="suggestion" key={task.id}><div><span className="minutes">{task.minutes} phút</span><h3>{task.title}</h3></div><button className="secondary" onClick={() => void onChoose(task)}>Chọn bước này</button></article>)}</section>}</> }
