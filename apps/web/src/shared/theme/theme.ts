...
export function initializeTheme() {
  ...
  initialized = true;
  try { preference = validTheme(localStorage.getItem(themeKey)); } catch { /* Storage is optional. */ }
  matchMedia("(prefers-color-scheme: dark)").addEventListener("change", apply);
  window.addEventListener("storage", event => { if (event.key === themeKey || event.key === null) { preference = validTheme(event.newValue); apply(); } });
  apply();
}
export function setTheme(value: Theme) {
  preference = value;
  try { localStorage.setItem(themeKey, value); } catch { /* Keep the in-memory choice. */ }
  apply();
}
export function useTheme() {
  return useSyncExternalStore(listener => { listeners.add(listener); return () => { listeners.delete(listener); }; }, () => preference);
}
